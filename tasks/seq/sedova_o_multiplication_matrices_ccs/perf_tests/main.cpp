...
TEST(sedova_o_multiplication_matrices_ccs_mpi, test_pipeline_run) {
  ...
  A_ = sedova_o_multiplication_matrices_ccs_seq::gen_rand_matrix(size, size, elements);
  B_ = sedova_o_multiplication_matrices_ccs_seq::gen_rand_matrix(size, size, elements);
  int rows_A = A_.size();
  ...
  sedova_o_multiplication_matrices_ccs_seq::convertMatrixToCCS(A_, rows_A, cols_A, A_val, A_row_ind, A_col_ptr);
  sedova_o_multiplication_matrices_ccs_seq::convertMatrixToCCS(B_, rows_B, cols_B, B_val, B_row_ind, B_col_ptr);

  std::vector<double> exp_C_val;
  std::vector<int> exp_C_row_ind;
  std::vector<int> exp_C_col_ptr;
  auto exp_C = sedova_o_multiplication_matrices_ccs_seq::multiply_matrices(A_, B_);
  sedova_o_multiplication_matrices_ccs_seq::convertMatrixToCCS(
  ...
  auto task = std::make_shared<sedova_o_multiplication_matrices_ccs_seq::MatrixMultiplicationCCS>(task_data);
...
TEST(sedova_o_multiplication_matrices_ccs_mpi, test_task_run) {
  ...
  std::vector<int> exp_C_col_ptr;
  auto exp_C = sedova_o_multiplication_matrices_ccs_seq::multiply_matrices(A_, B_);
  sedova_o_multiplication_matrices_ccs_seq::convertMatrixToCCS(exp_C, exp_C.size(), exp_C.empty() ? 0 : exp_C[0].size(),
