...
TEST(gordeva_t_max_val_of_column_matrix_seq, Max_val_of_10_columns_with_random) {
  // Create data
  const int rows = 10;
  const int cols = 10;
  ...
  std::shared_ptr<ppc::core::TaskData> taskDataSeq = std::make_shared<ppc::core::TaskData>();
  gordeva_t_max_val_of_column_matrix_seq::TestTaskSequential testTaskSequential(taskDataSeq);
  std::vector<std::vector<int>> matrix = gordeva_t_max_val_of_column_matrix_seq::TestTaskSequential::gen_rand_matr(rows, cols);
  for (auto &i : matrix) 
