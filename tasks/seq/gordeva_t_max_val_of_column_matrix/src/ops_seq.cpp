...
std::vector<int> gordeva_t_max_val_of_column_matrix_seq::TestTaskSequential::gen_rand_vec(int size, int lower_bound, int upper_bound) 
{
  std::vector<int> v(size);
  for (auto& number : v) 
      number = lower_bound + (std::rand() % (upper_bound - lower_bound + 1));
  return v;
}

std::vector<std::vector<int>> gordeva_t_max_val_of_column_matrix_seq::TestTaskSequential::gen_rand_matr(int rows, int cols) 
{
