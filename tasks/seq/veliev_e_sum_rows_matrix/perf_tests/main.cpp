...
TEST(veliev_e_sum_rows_matrix_seq, test_pipeline_run) {
  uint32_t rows = 3000;
  uint32_t cols = 3000;
  ...
  auto taskSeq = std::make_shared<veliev_e_sum_rows_matrix_seq::TestTaskSequential>(taskDataSeq);
...
TEST(veliev_e_sum_rows_matrix_seq, test_task_run) {
  uint32_t rows = 3000;
  uint32_t cols = 3000;
