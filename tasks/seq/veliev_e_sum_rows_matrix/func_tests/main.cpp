...
#include "seq/veliev_e_sum_rows_matrix/include/ops_seq.hpp"
...
TEST(veliev_e_sum_rows_matrix_seq, rectangular_matrix_stretched_horizontally_7x17) {
  uint32_t rows = 7;
  uint32_t cols = 17;
  ...
  veliev_e_sum_rows_matrix_seq::TestTaskSequential taskSeq(taskDataSeq);
...
TEST(veliev_e_sum_rows_matrix_seq, rectangular_matrix_stretched_verticaly_100x75) {
  uint32_t rows = 100;
  uint32_t cols = 75;

  std::vector<int32_t> seqSum(rows);
  std::vector<std::vector<int32_t>> randomData = veliev_e_sum_rows_matrix_test_function::getRandomData(rows, cols);
  std::shared_ptr<ppc::core::TaskData> taskDataSeq = std::make_shared<ppc::core::TaskData>();
  veliev_e_sum_rows_matrix_seq::TestTaskSequential taskSeq(taskDataSeq);
...
TEST(veliev_e_sum_rows_matrix_seq, squere_matrix_100x100) {
  uint32_t rows = 100;
  uint32_t cols = 100;
...
TEST(veliev_e_sum_rows_matrix_seq, matrix_1x1) {
  uint32_t rows = 1;
  uint32_t cols = 1;
...
TEST(veliev_e_sum_rows_matrix_seq, test_validation) {
  uint32_t rows = 7;
  uint32_t cols = 17;
