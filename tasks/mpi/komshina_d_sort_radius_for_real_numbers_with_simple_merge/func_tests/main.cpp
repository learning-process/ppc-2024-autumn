...
TEST(komshina_d_sort_radius_for_real_numbers_with_simple_merge_mpi, Test_Empty_Vector) {
  ...
  if (world.rank() == 0) {
    ASSERT_TRUE(sorted_empty_vec.empty()) << "Parallel: Empty vector was not handled correctly!";
  }
  ...
  if (world.rank() == 0) {
    ASSERT_TRUE(sorted_empty_vec.empty()) << "Sequential: Empty vector was not handled correctly!";
  }
}
