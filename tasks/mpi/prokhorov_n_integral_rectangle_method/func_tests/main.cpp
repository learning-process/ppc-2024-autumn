...
TEST(prokhorov_n_integral_rectangle_method_mpi, Test_Sum) {
  ...
  if (world.rank() == 0) {
    ...
    global_vec = prokhorov_n_integral_rectangle_method_mpi::getRandomVector(count_size_vector);
...
TEST(prokhorov_n_integral_rectangle_method_mpi, Test_Diff) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
...
TEST(prokhorov_n_integral_rectangle_method_mpi, Test_Diff_2) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_diff(1, 0);
  // Create TaskData
...
TEST(prokhorov_n_integral_rectangle_method_mpi, Test_Max) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  ...
  prokhorov_n_integral_rectangle_method_mpi::TestMPITaskParallel testMpiTaskParallel(taskDataPar, "max");
  ASSERT_EQ(testMpiTaskParallel.validation(), true);
  testMpiTaskParallel.pre_processing();
  testMpiTaskParallel.run();
  testMpiTaskParallel.post_processing();
  ...
  if (world.rank() == 0) {
    ...
    // Create Task
    prokhorov_n_integral_rectangle_method_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataSeq, "max");
    ASSERT_EQ(testMpiTaskSequential.validation(), true);
    testMpiTaskSequential.pre_processing();
    testMpiTaskSequential.run();
    testMpiTaskSequential.post_processing();
...
TEST(prokhorov_n_integral_rectangle_method_mpi, Test_Max_2) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_max(1, 0);
  // Create TaskData
