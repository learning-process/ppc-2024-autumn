...
TEST(chistov_a_gather_my, returns_empty_vector_when_small_size_2) {
  auto vector1 = chistov_a_gather_my::getRandomVector<int>(0);
  ASSERT_TRUE(vector1.empty());

  auto vector2 = chistov_a_gather_my::getRandomVector<int>(-1);
  ASSERT_TRUE(vector2.empty());

  auto vector3 = chistov_a_gather_my::getRandomVector<int>(1);
