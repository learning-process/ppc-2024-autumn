...
class MPIGaussVerticalSequential : public ppc::core::Task {
 public:
  explicit MPIGaussVerticalSequential(std::shared_ptr<ppc::core::TaskData> taskData_, int band_width_) : Task(std::move(taskData_)), band_width(band_width_) {}
...
class MPIGaussVerticalParallel : public ppc::core::Task {
 public:
  explicit MPIGaussVerticalParallel(std::shared_ptr<ppc::core::TaskData> taskData_, int band_width_) : Task(std::move(taskData_)), band_width(band_width_) {}
  bool pre_processing() override;
  bool validation() override;
  bool run() override;
  bool post_processing() override;
