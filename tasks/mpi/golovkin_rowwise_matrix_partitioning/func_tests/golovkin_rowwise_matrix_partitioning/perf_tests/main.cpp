...
TEST(golovkin_rowwise_matrix_partitioning, test_pipeline_run) {
  // Определение размера матриц для теста
  const int N = 500;  // Размер матрицы (500x500)

  // Заполнение матриц значениями
  std::vector<std::vector<double>> A(N, std::vector<double>(N, 1.0));       // Матрица A
  std::vector<std::vector<double>> B(N, std::vector<double>(N, 1.0));       // Матрица B
  std::vector<std::vector<double>> result(N, std::vector<double>(N, 0.0));  // Результат

  // Подготовка данных TaskData
  ...
  auto matrixMultiplicationTask =
      std::make_shared<MatrixMultiplicationTask>(taskDataSeq);  // Ваш класс для умножения матриц

  auto perfAttr = std::make_shared<ppc::core::PerfAttr>();
  perfAttr->num_running = 10;  // Количество повторений для измерений
  ...
  // Запуск выполнения теста с измерением производительности
  perfAnalyzer->pipeline_run(perfAttr, perfResults);

  // Вывод статистики
  ppc::core::Perf::print_perf_statistic(perfResults);

  // Проверка правильности результата (для простоты, можно проверить размер)
...
TEST(golovkin_rowwise_matrix_partitioning, test_task_run) {
  ...
  // Запуск теста с производительностью
  auto perfAnalyzer = std::make_shared<ppc::core::Perf>(matrixMultiplicationTask);
