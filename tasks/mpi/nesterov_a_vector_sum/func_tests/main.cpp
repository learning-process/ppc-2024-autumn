...
#include "mpi/nesterov_a_vector_sum/include/ops_mpi.hpp"

TEST(nesterov_a_vector_sum_mpi, test_sum) {
  ...
  if (world.rank() == 0) {
    ...
    global_vec = nesterov_a_vector_sum_mpi::getRandomVector(count_size_vector);
  ...
  nesterov_a_vector_sum_mpi::TestMPITaskParallel testMpiTaskParallel(taskDataPar, "+");
  ...
  if (world.rank() == 0) {
    ...
    // Create Task
    nesterov_a_vector_sum_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataSeq, "+");
...
TEST(nesterov_a_vector_sum_mpi, test_diff) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
...
TEST(nesterov_a_vector_sum_mpi, test_diff_2) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_diff(1, 0);
  // Create TaskData
...
TEST(nesterov_a_vector_sum_mpi, test_max) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  ...
  nesterov_a_vector_sum_mpi::TestMPITaskParallel testMpiTaskParallel(taskDataPar, "max");
  ASSERT_EQ(testMpiTaskParallel.validation(), true);
  testMpiTaskParallel.pre_processing();
  testMpiTaskParallel.run();
  testMpiTaskParallel.post_processing();
  ...
  if (world.rank() == 0) {
    ...
    // Create Task
    nesterov_a_vector_sum_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataSeq, "max");
    ASSERT_EQ(testMpiTaskSequential.validation(), true);
    testMpiTaskSequential.pre_processing();
    testMpiTaskSequential.run();
    testMpiTaskSequential.post_processing();
...
TEST(nesterov_a_vector_sum_mpi, test_max_2) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_max(1, 0);
  // Create TaskData
