...
TEST(kolokolova_d_max_of_vector_elements_mpi, Test1) {
...
TEST(kolokolova_d_max_of_vector_elements_mpi, Test2) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
...
TEST(kolokolova_d_max_of_vector_elements_mpi, Test3) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_diff(1, 0);
  // Create TaskData
...
TEST(kolokolova_d_max_of_vector_elements_mpi, Test4) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  ...
  kolokolova_d_max_of_vector_elements_mpi::TestMPITaskParallel testMpiTaskParallel(taskDataPar, "max");
  ASSERT_EQ(testMpiTaskParallel.validation(), true);
  testMpiTaskParallel.pre_processing();
  testMpiTaskParallel.run();
  testMpiTaskParallel.post_processing();
  ...
  if (world.rank() == 0) {
    ...
    // Create Task
    kolokolova_d_max_of_vector_elements_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataSeq, "max");
    ASSERT_EQ(testMpiTaskSequential.validation(), true);
    testMpiTaskSequential.pre_processing();
    testMpiTaskSequential.run();
    testMpiTaskSequential.post_processing();
...
TEST(kolokolova_d_max_of_vector_elements_mpi, Test5) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_max(1, 0);
  // Create TaskData
