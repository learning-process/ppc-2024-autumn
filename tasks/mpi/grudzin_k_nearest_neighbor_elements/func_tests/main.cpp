...
TEST(grudzin_k_nearest_neighbor_elements_mpi, Wrong_Test) {
  ...
  if (world.rank() == 0) {
    ...
    // Create Task
    grudzin_k_nearest_neighbor_elements_mpi::TestMPITaskSequential testMpiTaskSequential(taskDataSeq);
    ASSERT_EQ(testMpiTaskSequential.validation(), false);
  }
}

TEST(grudzin_k_nearest_neighbor_elements_mpi, Test_10k) {
  boost::mpi::communicator world;
  ...
  if (world.rank() == 0) {
    ...
    ASSERT_EQ(reference_ans[0], global_ans[0]);
  }
}

TEST(grudzin_k_nearest_neighbor_elements_mpi, Test_1k) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_ans(1, INT_MAX);
  // Create TaskData
...
TEST(grudzin_k_nearest_neighbor_elements_mpi, Test_2k) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_ans(1, INT_MAX);
  // Create TaskData
...
TEST(grudzin_k_nearest_neighbor_elements_mpi, Test_4k) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_ans(1, INT_MAX);
  // Create TaskData
  ...
  if (world.rank() == 0) {
    const int count_size_vector = 4000;
    global_vec = grudzin_k_nearest_neighbor_elements_mpi::getRandomVector(count_size_vector);
    taskDataPar->inputs.emplace_back(reinterpret_cast<uint8_t*>(global_vec.data()));
    taskDataPar->inputs_count.emplace_back(global_vec.size());
    taskDataPar->outputs.emplace_back(reinterpret_cast<uint8_t*>(global_ans.data()));
...
TEST(grudzin_k_nearest_neighbor_elements_mpi, Test_3k) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_ans(1, INT_MAX);
  // Create TaskData
...
TEST(grudzin_k_nearest_neighbor_elements_mpi, Test_3) {
  boost::mpi::communicator world;
  std::vector<int> global_vec;
  std::vector<int32_t> global_ans(1, INT_MAX);
  // Create TaskData
