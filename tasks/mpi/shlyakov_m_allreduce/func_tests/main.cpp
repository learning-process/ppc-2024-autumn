...
TEST(shlyakov_m_all_reduce_mpi, allreduce_test_with_matrix_100_100) {
  int col = 100;
  int row = 100;
  ...
  if (world.rank() == 0) {
    ...
    ASSERT_EQ(m_vec.size(), rm_vec.size());
    for (size_t i = 0; i < m_vec.size(); ++i) {
      ASSERT_EQ(m_vec[i], rm_vec[i]);
    }
  }
}

TEST(shlyakov_m_all_reduce_mpi, Test_0_0_1) {
  int row = 3;
  int col = 6;
  boost::mpi::communicator world;
  std::vector<int> matrix;
...
TEST(shlyakov_m_all_reduce_mpi, My_Test_0_0_1) {
  int row = 3;
  int col = 6;
  boost::mpi::communicator world;
  std::vector<int> matrix;
