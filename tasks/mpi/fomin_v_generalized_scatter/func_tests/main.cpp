...
TEST(GeneralizedScatterTest, ScatterIntegers) {
    ...
    int root = 0;
    const int data_size = size * 10; // Ensure sendcount is divisible by size
    int* sendbuf = nullptr;
    ...
    fomin_v_generalized_scatter::generalized_scatter(sendbuf, data_size, MPI_INT, recvbuf, 10, MPI_INT, root, MPI_COMM_WORLD);
...
TEST(GeneralizedScatterTest, ScatterFloats) {
    ...
    fomin_v_generalized_scatter::generalized_scatter(sendbuf, data_size, MPI_FLOAT, recvbuf, 10, MPI_FLOAT, root, MPI_COMM_WORLD);
...
TEST(GeneralizedScatterTest, ScatterDoubles) {
    ...
    fomin_v_generalized_scatter::generalized_scatter(sendbuf, data_size, MPI_DOUBLE, recvbuf, 10, MPI_DOUBLE, root, MPI_COMM_WORLD);
...
TEST(GeneralizedScatterTest, SingleProcessScatter) {
    ...
    fomin_v_generalized_scatter::generalized_scatter(sendbuf, data_size, MPI_INT, recvbuf, data_size, MPI_INT, root, MPI_COMM_WORLD);
...
TEST(GeneralizedScatterTest, ZeroElementsScatter) {
    ...
    fomin_v_generalized_scatter::generalized_scatter(sendbuf, data_size, MPI_INT, recvbuf, 0, MPI_INT, root, MPI_COMM_WORLD);
