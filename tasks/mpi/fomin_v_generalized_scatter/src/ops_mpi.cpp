...
namespace fomin_v_generalized_scatter {
    ...
    int generalized_scatter(const void* sendbuf, int sendcount, MPI_Datatype sendtype,
        ...
        if (rank == root) {
            for (int child = left_child; child < size; child = right_child + 1) {
                if (child >= size)
                    break;
                int err = MPI_Send(sendbuf + (child * recvcount * MPI_Type_size(recvtype)),
                                   recvcount, sendtype, child, 0, comm);
                if (err != MPI_SUCCESS) {
    ...
    public:
        ...
        bool run(TaskData* taskData) {
            ...
            if (rank == root) {
                int err = generalized_scatter(taskData->inputs[0], sendcount, taskData->datatype,
                                              taskData->outputs[0], recvcount, taskData->datatype,
                                              root, MPI_COMM_WORLD);
                if (err != MPI_SUCCESS) {
                ...
                int err = generalized_scatter(nullptr, 0, taskData->datatype,
                                              taskData->outputs[0], recvcount, taskData->datatype,
                                              root, MPI_COMM_WORLD);
                if (err != MPI_SUCCESS) {
        ...
        bool post_processing(TaskData* taskData) {
            internal_order_test();
            // Логика для завершения параллельной обработки
