...
bool deryabin_m_cannons_algorithm_mpi::CannonsAlgorithmMPITaskParallel::run() {
  ...
  if (world.rank() == 0) {
      ...
      while (j != block_rows_columns) {
        if (i != 0 && j != 0) {
          ...
          while (k != block_dimension) {
            ...
            if (j == 0) {
              world.send(i * block_rows_columns + j, k + 1,
                         input_matrix_B.data() + (i * block_dimension + k) * dimension + j * block_dimension,
                         block_dimension);
            } else {
              if ((i - j) * block_rows_columns + j < 0) {
                world.send((i + block_rows_columns - j) * block_rows_columns + j, k + 1,
                           input_matrix_B.data() + (i * block_dimension + k) * dimension + j * block_dimension,
                           block_dimension);
              } else {
                world.send((i - j) * block_rows_columns + j, k + 1,
                           input_matrix_B.data() + (i * block_dimension + k) * dimension + j * block_dimension,
                           block_dimension);
              }
            }
    ...
    while (k != block_dimension) {
      ...
      world.recv(0, k + 1, local_input_matrix_B.data() + k * block_dimension, block_dimension);
      k++;
    }
  }
  i = 0;
