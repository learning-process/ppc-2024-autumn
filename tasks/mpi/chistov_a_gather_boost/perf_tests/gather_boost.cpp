...
TEST(chistov_a_gather_boost, test_pipeline_run2) {
  ...
  auto testMpiTaskParallel = std::make_shared<chistov_a_gather_boost::Gather<int>>(taskDataPar, 0);
...
TEST(chistov_a_gather_boost, test_task_run2) {
  boost::mpi::communicator world;
  const int count_size_vector = 50000000;
  std::vector<int> local_vec(count_size_vector, 1);
  std::vector<int> gathered_data;
