...
TEST(chistov_a_gather_boost, boost_task_check_char) {
  boost::mpi::communicator world;
  const int count_size_vector = 10;
  ...
  chistov_a_gather_boost::Reference<char> testMpiTaskParallel(taskDataPar);
  ASSERT_EQ(testMpiTaskParallel.validation(), true);
  testMpiTaskParallel.pre_processing();
  testMpiTaskParallel.run();
  testMpiTaskParallel.post_processing();
...
TEST(chistov_a_gather_boost, test_count_is_a_powers_of_two_sort) {
  boost::mpi::communicator world;
...
TEST(chistov_a_gather_boost, test_count_is_a_prime_number_sort) {
  boost::mpi::communicator world;
