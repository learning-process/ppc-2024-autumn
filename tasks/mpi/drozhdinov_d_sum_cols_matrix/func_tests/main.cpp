...
TEST(drozhdinov_d_sum_cols_matrix_mpi, RandomMatrixTest) {
  ...
  // Create data
  std::vector<int> matrix = drozhdinov_d_sum_cols_matrix_mpi::getRandomVector(c * r);
  std::vector<int> v = drozhdinov_d_sum_cols_matrix_mpi::calculateMatrixSumSequentially(matrix, c, r);
  std::vector<int> expres_par(c, 0);
  ...
  if (world.rank() == 0) {
    ...
    taskDataPar->inputs_count.emplace_back(c);
    taskDataPar->inputs_count.emplace_back(r);
