...
TEST(drozhdinov_d_sum_cols_matrix_mpi, WrongValidationTestPar) {
  boost::mpi::communicator world;
  ...
  if (world.rank() == 0) {
    ...
    taskDataPar->inputs_count.emplace_back(rows);
    taskDataPar->outputs.emplace_back(reinterpret_cast<uint8_t*>(expres_par.data()));
    taskDataPar->outputs_count.emplace_back(matrix.size());
  }
...
TEST(drozhdinov_d_sum_cols_matrix_mpi, WrongValidationTestSeq) {
  boost::mpi::communicator world;
