...
TEST(yasakova_t_min_of_vector_elements_mpi, testFindMinimumInEmptyMatrix) {
  ...
  if (world.rank() == 0) {
    ...
    if (!testMpiTaskSequential.validation()) {
      reference_minimum[0] = INT_MAX; 
    } else {
...
TEST(yasakova_t_min_of_vector_elements_mpi, testFindMinimumIn1xNMatrix) {
  ...
  if (world.rank() == 0) {
    ...
    taskDataSeq->inputs.emplace_back(reinterpret_cast<uint8_t*>(&global_matrix[0][0])); 
