...
TEST(zaitsev_a_scatter, test_pipeline_run) {
  ...
  auto testMpiTaskParallel = std::make_shared<zaitsev_a_scatter::TestMPITaskParallel<int>>(taskDataPar, root, MPI_INT);
...
TEST(zaitsev_a_scatter, test_task_run) {
  boost::mpi::communicator world;
  std::vector<int> inp_vector;
  int sz;
  int extrema = -1;
