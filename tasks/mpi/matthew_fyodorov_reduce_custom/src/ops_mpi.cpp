...
bool matthew_fyodorov_reduce_custom_mpi ::TestMPITaskSequential::validation() {
  internal_order_test();

  return taskData->outputs_count[0] == 1;
}

bool matthew_fyodorov_reduce_custom_mpi ::TestMPITaskSequential::run() {
  internal_order_test();
  if (ops == "+") {
...
bool matthew_fyodorov_reduce_custom_mpi ::TestMPITaskSequential::post_processing() {
  internal_order_test();
  reinterpret_cast<int*>(taskData->outputs[0])[0] = res;
  return true;
}

bool matthew_fyodorov_reduce_custom_mpi ::TestMPITaskParallel::pre_processing() {
  internal_order_test();
  if (world.rank() == 0) {
...
bool matthew_fyodorov_reduce_custom_mpi ::TestMPITaskParallel::run() {
  internal_order_test();
  int rank = world.rank();
...
bool matthew_fyodorov_reduce_custom_mpi ::TestMPITaskParallel::post_processing() {
  internal_order_test();
  if (world.rank() == 0) {
    reinterpret_cast<int*>(taskData->outputs[0])[0] = res;
  }
