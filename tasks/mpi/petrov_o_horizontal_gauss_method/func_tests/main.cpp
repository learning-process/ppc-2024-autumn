...
TEST(petrov_o_horizontal_gauss_method_par, TestGauss_RandomMatrix) {
    ...
    size_t n = 10; // Размер матрицы

    std::vector<double> random_matrix(n * n);
    std::vector<double> random_b(n);
    std::vector<double> par_output(n);
    std::vector<double> seq_output(n); // Выходные данные последовательной версии
    ...
    // Выполнение последовательной версии для получения эталонного результата
    ...
    // Сравнение результатов параллельной и последовательной версий
    if (world.rank() == 0) {
       for (size_t i = 0; i < n; ++i) {
...
TEST(petrov_o_horizontal_gauss_method_seq, TestGauss_Simple) {
    ...
    petrov_o_horizontal_gauss_method_mpi::SequentialTask task(taskData); // Изменено имя класса
