...
TEST(ermilova_d_reduceMPI_mpi, test_pipeline_run) {
  std::random_device dev;
  std::mt19937 gen(dev());
  ...
  if (world.rank() == 0) {
    ...
    global_matrix = ermilova_d_reduceMPI_mpi::getRandomMatrix(rows, cols, upper_border, lower_border);
  ...
  auto testMpiTaskParallel = std::make_shared<ermilova_d_reduceMPI_mpi::TestMPITaskParallel>(taskDataPar);
...
TEST(ermilova_d_reduceMPI_mpi, test_task_run) {
  std::random_device dev;
  std::mt19937 gen(dev());
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
