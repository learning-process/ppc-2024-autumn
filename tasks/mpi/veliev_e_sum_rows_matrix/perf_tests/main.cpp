...
TEST(veliev_e_sum_rows_matrix_mpi, test_pipeline_run) {
  uint32_t rows = 3000;
  uint32_t cols = 3000;
  ...
  auto testMpiTaskParallel = std::make_shared<veliev_e_sum_rows_matrix_mpi::TestTaskParallel>(taskDataPar);
...
TEST(veliev_e_sum_rows_matrix_mpi, test_task_run) {
  uint32_t rows = 3000;
  uint32_t cols = 3000;
