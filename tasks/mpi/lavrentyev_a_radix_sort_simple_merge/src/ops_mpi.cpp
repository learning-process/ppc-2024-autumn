...
void mergeSortedVectorsInPlace(std::vector<double>& arr, const std::vector<int>& sizes,
...
bool lavrentyev_a_radix_sort_simple_merge_mpi::RadixSimpleMerge::run() {
  ...
  if (wrank == 0) {
    ...
    mergeSortedVectorsInPlace(arr, sizes, displs);
  } else {
    boost::mpi::gatherv(world, local_arr.data(), local_arr.size(), 0);
  }
