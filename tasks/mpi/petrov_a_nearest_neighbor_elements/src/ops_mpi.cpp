...
bool petrov_a_nearest_neighbor_elements_mpi::TestMPITaskSequential::pre_processing() {
  ...
  closest_pair_ = {0, 1};  // Ќачальное значение
  return true;
}
...
bool petrov_a_nearest_neighbor_elements_mpi::TestMPITaskParallel::run() {
  ...
  // ”бедитесь, что вы устанавливаете closest_pair_ в глобальную пару
  if (world.rank() == 0) {
    closest_pair_ = global_pair;  // ≈сли это корень, присваиваем глобальную пару
  }
