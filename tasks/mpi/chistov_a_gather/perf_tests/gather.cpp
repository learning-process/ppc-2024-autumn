...
TEST(chistov_a_gather, test_pipeline_run) {
  ...
  auto testMpiTaskParallel = std::make_shared<chistov_a_gather::Gather<int>>(taskDataPar, 0);
...
TEST(chistov_a_gather, test_task_run) {
  boost::mpi::communicator world;
  const int count_size_vector = 100000000;
  std::vector<int> local_vec(count_size_vector, 1);
  std::vector<int> gathered_data;
