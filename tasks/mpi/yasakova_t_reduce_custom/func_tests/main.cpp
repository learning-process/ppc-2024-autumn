...
void Test_reduce(MPI_Datatype datatype, MPI_Op op) {
  ...
  yasakova_t_reduce_custom_mpi::CustomReduce(&local_result, &global_result, 1, datatype, op, 0, MPI_COMM_WORLD);
...
TEST(yasakova_t_reduce_custom_mpi, Can_create_vector) {
  const int size_test = 10;

  EXPECT_NO_THROW(getRandom<int>(size_test));
}

TEST(yasakova_t_reduce_custom_mpi, Cant_create_incorrect_size_vector) {
  const int size_test = -10;
...
TEST(yasakova_t_reduce_custom_mpi, CustomReduce_int_min) { Test_reduce<int>(MPI_INT, MPI_MIN); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_int_max) { Test_reduce<int>(MPI_INT, MPI_MAX); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_float_sum) { Test_reduce<float>(MPI_FLOAT, MPI_SUM); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_float_min) { Test_reduce<float>(MPI_FLOAT, MPI_MIN); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_float_max) { Test_reduce<float>(MPI_FLOAT, MPI_MAX); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_double_sum) { Test_reduce<double>(MPI_DOUBLE, MPI_SUM); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_double_min) { Test_reduce<double>(MPI_DOUBLE, MPI_MIN); }

TEST(yasakova_t_reduce_custom_mpi, CustomReduce_double_max) { Test_reduce<double>(MPI_DOUBLE, MPI_MAX); }

TEST(yasakova_t_reduce_custom_mpi, Matrix_1x1) {
  boost::mpi::communicator world;
  ...
  yasakova_t_reduce_custom_mpi::TestMPITaskParallel testMpiTaskParallel(taskDataPar);
...
TEST(yasakova_t_reduce_custom_mpi, Matrix_10x10) {
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
  std::vector<int32_t> global_min(1, INT_MAX);
  const int rows_test = 10;
...
TEST(yasakova_t_reduce_custom_mpi, Matrix_100x100) {
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
  std::vector<int32_t> global_min(1, INT_MAX);
...
TEST(yasakova_t_reduce_custom_mpi, Matrix_100x50) {
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
  std::vector<int32_t> global_min(1, INT_MAX);
  const int rows_test = 100;
...
TEST(yasakova_t_reduce_custom_mpi, Matrix_50x100) {
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
  std::vector<int32_t> global_min(1, INT_MAX);
...
TEST(yasakova_t_reduce_custom_mpi, Matrix_500x500) {
  boost::mpi::communicator world;
  std::vector<std::vector<int>> global_matrix;
  std::vector<int32_t> global_min(1, INT_MAX);
