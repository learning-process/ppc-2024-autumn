...
bool yasakova_t_reduce_custom_mpi::TestMPITaskParallel::validation() {
  internal_order_test();
  if (world.rank() == 0) {
...
bool yasakova_t_reduce_custom_mpi::TestMPITaskParallel::run() {
  internal_order_test();
...
bool yasakova_t_reduce_custom_mpi::TestMPITaskParallel::post_processing() {
  internal_order_test();
  if (world.rank() == 0) {
...
int yasakova_t_reduce_custom_mpi::CustomReduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
