...
TEST(alputov_i_most_diff_neighb_elem_mpi, Test_MaxDiff_RandomLargeVector_MPI_1000_elem) {
  ...
  if (world.rank() == 0) {
    inputVector =
        alputov_i_most_diff_neighb_elem_mpi::RandomVector(1000);
...
TEST(alputov_i_most_diff_neighb_elem_mpi, Test_MaxDiff_RandomVector_MPI_100_elem) {
  ...
  if (world.rank() == 0) {
    inputVector =
        alputov_i_most_diff_neighb_elem_mpi::RandomVector(100);  
...
TEST(alputov_i_most_diff_neighb_elem_mpi, Test_MaxDiff_LargeVector_MPI_with_equal_elem) {
  boost::mpi::communicator world;
  std::vector<int> inputVector(1000);
  int outputPair[2] = {};
