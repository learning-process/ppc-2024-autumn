...
bool budazhapova_e_matrix_mult_mpi::MatrixMultSequential::pre_processing() {
	internal_order_test();
	A = std::vector<int>(reinterpret_cast<int*>(taskData->inputs[0]), reinterpret_cast<int*>(taskData->inputs[0]) + taskData->inputs_counts[0]);
	b = std::vector<int>(reinterpret_cast<int*>(taskData->inputs[1]), reinterpret_cast<int*>(taskData->inputs[1]) + taskData->inputs_counts[1]);
...
bool budazhapova_e_matrix_mult_mpi::MatrixMultSequential::validation() {
	internal_order_test();
	return double(taskData->inputs_count[0]) % columns == 0 && taskData->inputs_counts[0] > 0 && taskData->inputs_counts[1] > 0;
}
...
bool budazhapova_e_matrix_mult_mpi::MatrixMultParallel::validation() {
	...
	if (world.rank() == 0)
	{
		return double(taskData->inputs_count[0]) % columns == 0 && taskData->inputs_counts[0] > 0 && taskData->inputs_counts[1] > 0 && taskData->outputs_count[0] == (taskData->inputs_counts[0] / taskData->inputs_counts[1]);
	}
	return true;
}
