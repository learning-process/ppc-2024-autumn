...
TEST(dudchenko_o_sleeping_barber_mpi, Test_Validation1) {
  ...
  dudchenko_o_sleeping_barber_mpi::TestMPISleepingBarber TestMPISleepingBarber(taskDataPar);

  if (world.rank() == 0) {
    taskDataPar->inputs_count = {0};
    EXPECT_FALSE(TestMPISleepingBarber.validation());
  }
}
...
TEST(dudchenko_o_sleeping_barber_mpi, Test_Validation3) {
  ...
  if (world.rank() == 0) {
    if (world.size() < 3) {
      ...
      taskDataPar->inputs_count = {1};
      EXPECT_TRUE(TestMPISleepingBarber.validation());
    }
  }
}
...
TEST(dudchenko_o_sleeping_barber_mpi, Test_End_To_End1) {
  ...
  if (world.size() < 3) {
    ASSERT_FALSE(TestMPISleepingBarber.validation());
  } else {
    ASSERT_TRUE(TestMPISleepingBarber.validation());
    ASSERT_TRUE(TestMPISleepingBarber.pre_processing());
    ASSERT_TRUE(TestMPISleepingBarber.run());
    ASSERT_TRUE(TestMPISleepingBarber.post_processing());
