...
std::vector<double> generateRandomMatrixLinear(uint32_t size, double min_value, double max_value) {
  std::random_device dev;
  std::mt19937 gen(dev());
  std::uniform_real_distribution<double> dist(min_value, max_value);
...
TEST(malyshev_conjugate_gradient, test_small_system) {
  ...
  if (world.rank() == 0) {
    ...
    randomMatrix = malyshev_conjugate_gradient::generateRandomMatrixLinear(size, min_value, max_value);
    ...
    for (uint32_t i = 0; i < size; ++i) {
      for (uint32_t j = 0; j < size; ++j) {
        std::cerr << std::fixed << std::setprecision(6) << randomMatrix[i * size + j] << " ";
      }
      std::cerr << std::endl;
    }
