...
bool frolova_e_matrix_multiplication_mpi::matrixMultiplicationParallel::run() {
  internal_order_test();
   ...
   if (world.rank() == 0) {
       ...
       for (unsigned int i = 1; i < active_processes; ++i) {
           ...
           if (line_data.numberOfLines > 0) {
             ...
             line_data.local_lines.clear();
             line_data.index_lines.clear();
             line_data.res_lines.clear(); 
             line_data.outgoingLineLength = 0;
           }           
           ...
           if (column_data.numberOfColumns > 0) {
             ...
             column_data.local_columns.clear();
             column_data.index_colums.clear();
   ...
   if (world.rank() > 0 && world.rank() < static_cast<int>(active_processes)) {
       ...
       //if (localLinesA.numberOfLines > 0) {
       //    std::cout << "Process " << world.rank() << " received lines data." << std::endl;
       //} else {
       //    std::cout << "Process " << world.rank() << " has no lines assigned." << std::endl;
       //}

       world.recv(0, 0, localColumnB);

       //if (localColumnB.numberOfColumns > 0) {
       //    std::cout << "Process " << world.rank() << " received columns data." << std::endl;
       //} else {
       //    std::cout << "Process " << world.rank() << " has no columns assigned." << std::endl;
       //}
   ...
   if (world.rank() < static_cast<int>(active_processes) &&
       (active_processes > 1)) {
       ...
       for (int step = 0; step < static_cast<int>(active_processes); ++step) {
           ...
           /*world.send(next_process, 0, localColumnB);
           std::cout << "process :" << world.rank() << " send columns" << std::endl;
           world.recv(prev_process, 0, localColumnB);
           std::cout << "process :" << world.rank() << " recev columns" << std::endl;

           std::cout << "Process " << world.rank() << " step " << step << " sending to " << next_process
                     << " receiving from " << prev_process << std::endl;*/
